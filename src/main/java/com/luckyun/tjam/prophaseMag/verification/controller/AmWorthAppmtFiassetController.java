package com.luckyun.tjam.prophaseMag.verification.controller;


import org.apache.ibatis.annotations.Param;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;

import com.luckyun.core.annotation.UrlByPkgController;
import com.luckyun.core.api.annotation.Api;
import com.luckyun.tjam.prophaseMag.verification.model.AmWorthAppmtBatch;
import com.luckyun.tjam.prophaseMag.verification.param.AmWorthAppmtFiassetParam;
import com.luckyun.tjam.prophaseMag.verification.service.AmWorthAppmtFiassetService;

@Api(name = "待分摊项维护模块控制层", author = "cy")
@UrlByPkgController
public class AmWorthAppmtFiassetController {

    @Autowired
    private AmWorthAppmtFiassetService amWorthAppmtFiassetService;

    @Api(name = "批量删除操作", author = "cy")
    @PostMapping("delete")
    public void delOpr(@Param("核验计划主数据") @RequestBody AmWorthAppmtFiassetParam condition) {
        this.amWorthAppmtFiassetService.del(condition);
    }

    @Api(name = "新增子表明细", author = "cy")
    @PostMapping("add")
    public void addAwaFiasset(@Param("新增数据") @RequestBody AmWorthAppmtBatch entity){
        amWorthAppmtFiassetService.addAwaFiasset(entity);
    }

    @Api(name = "修改分摊金额",author = "whs")
    @PostMapping("update")
    public void update(@RequestBody AmWorthAppmtFiassetParam entity){
        amWorthAppmtFiassetService.updateFiasset(entity);
    }
}
